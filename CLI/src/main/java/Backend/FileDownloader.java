...
/**
 * This class deals with downloading the file for Drifty CLI.
 */
public class FileDownloader implements Runnable {
    ...
    public void downloadFromYouTube() {
        ...
        ProcessBuilder processBuilder = new ProcessBuilder(Program.get(YT_DLP_PATH), "--quiet", "--progress", "-P", dir, link, "-o", outputFileName);
    ...
    private void downloadFromInstagram() throws InterruptedException, IOException {
        ...
        M.msgDownloadInfo("Trying to download \"" + fileDownloadMessage + "\" ...");
        ProcessBuilder processBuilder = new ProcessBuilder(Program.get(YT_DLP_PATH), "--quiet", "--progress", "-P", dir, link, "-o", outputFileName); // The command line arguments tell `yt-dlp` to download the video and to save it to the specified directory.
        processBuilder.inheritIO();
        M.msgDownloadInfo(String.format(DOWNLOADING_F, fileDownloadMessage));
