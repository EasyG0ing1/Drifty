...
import GUI_Support.Job;
import GUI_Support.DownloadMetrics;
import GUI_Init.Environment;
import GUI_Utils.MessageBroker;
import GUI_Utils.Utility;
...
public class DownloadFile extends Task<Integer> {
    ...
    private final String YT_DLP = Program.get(Program.YT_DLP_PATH);
    ...
    private Runnable split(DownloadMetrics downloadMetrics) {
        return () -> {
            InputStream in = null;
            HttpURLConnection con;
            FileOutputStream fos = null;
            try {
                URL url = downloadMetrics.getUrl();
                long start = downloadMetrics.getStart();
                long end = downloadMetrics.getEnd();
                fos = downloadMetrics.getFileOutputStream();
                ...
                while ((bytesRead = in.read(buffer)) != -1) {
                    ...
                    if (downloadMetrics.hasStopped()) {
                        downloadMetrics.setFailed(true);
                        break;
                    }
                }
    ...
    private void splitDownload() {
        ...
        try {
            int numParts = new Support.DownloadMetrics().getThreadCount();
            ...
            LinkedList<DownloadMetrics> list = new LinkedList<>();
            for (int x = 0; x < numParts; x++) {
                long startByte = (x == 0) ? 0 : (x * partSize) + 1;
                long endByte = ((numParts - 1) == x) ? fileSize : ((x * partSize) + partSize);
                DownloadMetrics partDownloadMetrics = new DownloadMetrics(x, startByte, endByte, filename, url);
                list.addLast(partDownloadMetrics);
                new Thread(split(partDownloadMetrics)).start();
            }
            ...
            while (loop) {
                ...
                for (DownloadMetrics partDownloadMetrics : list) {
                    if (partDownloadMetrics.isFailed())
                        stopThreads = true;
                    if (partDownloadMetrics.isRunning())
                        allDone = false;
                }
                if (stopThreads) {
                    for (DownloadMetrics partDownloadMetrics : list) {
                        partDownloadMetrics.setStop(true);
                    }
                }
